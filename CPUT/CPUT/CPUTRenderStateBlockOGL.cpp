...
void CPUTRenderStateBlockOGL::ReadValue( CPUTConfigEntry *pValue, const CPUTRenderStateMapEntry *pRenderStateList, void *pDest )
{
    ...
    for( CPUTRenderStateMapEntry const *pCur = pRenderStateList; pCur->name.compare(_L("")); pCur++ )
    {
        ...
        if( found )
        {
            ...
            case ePARAM_TYPE_STRING:        pValue->ValueAsString((cString*)&((char*)pDest)[pCur->offset]); break;
...
#ifndef CPUT_FOR_OGLES
    mStateDesc.RasterizerDesc.DepthClipEnable       ? glEnable(GL_DEPTH_CLAMP)  : glDisable(GL_DEPTH_CLAMP);
