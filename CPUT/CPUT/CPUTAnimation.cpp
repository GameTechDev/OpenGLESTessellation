...
cString CPUTNodeAnimation::GetTargetName() const
{
    return mTarget;
}

cString CPUTNodeAnimation::GetName() const
{
    return mName;
}
...
CPUTAnimation * CPUTAnimation::CreateAnimation( const cString &file )
{
    std::string name;
    //TODO: ws2s causing leak
    CPUTFileSystem::CPUTOSifstream stream(cString(file.begin(), file.end()),ios::binary);
    ...
    CPUTAssetLibrary::GetAssetLibrary()->AddAnimationSet(file, _L(""), _L(""), pAnimationSet);
