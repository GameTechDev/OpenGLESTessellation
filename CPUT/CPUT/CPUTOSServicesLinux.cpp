...
//-----------------------------------------------------------------------------
CPUTResult CPUTFileSystem::GetWorkingDirectory(cString *pPath)
{
	assert(false);
	/*
...
//-----------------------------------------------------------------------------
CPUTResult CPUTFileSystem::SetWorkingDirectory(const cString &path)
{
	assert(false);
	/*
...
//-----------------------------------------------------------------------------
CPUTResult CPUTFileSystem::GetExecutableDirectory(cString *pExecutableDir)
{
...
//-----------------------------------------------------------------------------
CPUTResult CPUTFileSystem::SplitPathAndFilename(const cString &sourceFilename, cString *pDrive, cString *pDir, cString *pFileName, cString *pExtension)
{
	assert(false);
	/*
...
CPUTResult CPUTFileSystem::ReadFileContents(const cString &fileName, UINT *pSizeInBytes, void **ppData, bool bAddTerminator, bool bLoadAsBinary)
{
    ...
    cString file = cString(_L(""));
    cString dir = cString(_L(""));
    if(index == -1)
    {
        file = fileName;
    }
...
void CPUTFileSystem::CPUTandroidifstream::read(char* s, int n)
{
    if (mbEOF || !mpAsset)
        return;
