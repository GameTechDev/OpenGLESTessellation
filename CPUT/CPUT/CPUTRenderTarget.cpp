...
)
{
    ...
    if( mMultiSampleCount>1 )
    {
        ...
        if( !recreate )
        {
            ...
            pAssetLibrary->AddTexture( finalName, _L(""), _L(""), mpColorTextureMSAA );
        }
    ...
    if( !recreate )
    {
        ...
        pAssetLibrary->AddTexture( mName, _L(""), _L(""), mpColorTexture );
        ...
        pAssetLibrary->AddBuffer( mName, _L(""), _L(""), mpColorBuffer );
    }
...
void CPUTRenderTargetColor::SaveRenderTargetToFile( CPUTRenderParameters &renderParams, cString fileName, int format )
{
    ...
    HRESULT hr = DirectX::CaptureTexture(pD3dDevice, ((CPUTRenderParametersDX*)&renderParams)->mpContext, mpColorTextureDX, imageResult);
    ASSERT(SUCCEEDED(hr), _L("Failed to capture texture"));

    const DirectX::Image* img = imageResult.GetImages();
    ASSERT(img, _L("Failed to retrieve images"));

    size_t nimg = imageResult.GetImageCount();
    ASSERT( nimg > 0, _L("No images retrieved") );
    ...
    hr = SaveToDDSFile( img, nimg, imageResult.GetMetadata(), DirectX::DDS_FLAGS_NONE, fileName.c_str() );
...
)
{
    ...
    if( supportsResourceView )
    {
        ...
        if( !recreate )
        {
            ...
            pAssetLibrary->AddTexture( mName, _L(""), _L(""), mpDepthTexture );
        }
...
void CPUTRenderTargetDepth::SaveRenderTargetToFile( CPUTRenderParameters &renderParams, cString fileName, int format )
{
    ID3D11Device *pD3dDevice = CPUT_DX11::GetDevice();
    DirectX::ScratchImage imageResult;
    HRESULT hr = DirectX::CaptureTexture(pD3dDevice, ((CPUTRenderParametersDX*)&renderParams)->mpContext, mpDepthTextureDX, imageResult);
    ASSERT(SUCCEEDED(hr), _L("Failed to capture texture"));
    ...
    hr = SaveToDDSFile( img, nimg, metaData, DirectX::DDS_FLAGS_NONE, fileName.c_str() );
