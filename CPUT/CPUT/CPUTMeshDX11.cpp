...
//-----------------------------------------------------------------------------
void CPUTMeshDX11::BindVertexShaderLayout(
    CPUTMaterialEffect *pMaterial, // Pointer to this mesh's submaterial array (an array of pointers to CPUTMaterial)
...
)
{
    ...
    // Bind the new material's vertex shader and this mesh.
    if( pMaterial )
    {
        // Get the vertex layout for this shader/format comb
        // If already exists, then GetLayout() returns the existing layout for reuse.
        CPUTVertexShaderDX11 *pVertexShader = ((CPUTMaterialEffectDX11*)pMaterial)->GetVertexShader();
        CPUTInputLayoutCacheDX11::GetInputLayoutCache()->GetLayout(pDevice, mpLayoutDescription, pVertexShader, pLayout);
    }
}
