...
//-----------------------------------------------------------------------------
CPUTFont *CPUTFontOGL::CreateFont( cString FontName, cString AbsolutePathAndFilename )
{
    
    CPUTFontOGL *pNewFont = new CPUTFontOGL();
    /*
    // load the actual font image
    CPUTAssetLibraryOGL *pAssetLibrary = (CPUTAssetLibraryOGL*)CPUTAssetLibraryOGL::GetAssetLibrary();
    pNewFont->mpTextAtlas= NULL;//(CPUTTextureOGL*) pAssetLibrary->GetTexture(AbsolutePathAndFilename, true);
    //ASSERT(pNewFont->mpTextAtlas, _L("CPUTFontOGL::CreateFont() - Error loading specified font texture"));
...
*/
    ...
    // Get and store the atlas size
    int width, height;
    //pNewFont->mpTextAtlas->GetWidthAndHeight(&width, &height);
    pNewFont->mAtlasWidth = 750.0f;//(float)width;
    pNewFont->mAtlasHeight = 24.0f;//(float)height;
    ...
    */
    ...
    CPUTAssetLibrary::GetAssetLibrary()->AddFont( _L(""), FontName, _L(""), pNewFont);
...
// Constructor
//-----------------------------------------------------------------------------
CPUTFontOGL::CPUTFontOGL():mpTextAtlas(NULL)//,
...
//-----------------------------------------------------------------------------
CPUTTextureOGL* CPUTFontOGL::GetAtlasTexture()
{
    return mpTextAtlas;
}
...
//-----------------------------------------------------------------------------
ID3D11ShaderResourceView* CPUTFontOGL::GetAtlasTextureResourceView()
{
...
//--------------------------------------------------------------------------------
CPUTResult CPUTFontOGL::LoadGlyphMappingFile(const cString fileName)
{
    return CPUT_SUCCESS;
}
