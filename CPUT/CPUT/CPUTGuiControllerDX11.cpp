...
void CPUTGuiControllerDX11::Draw(ID3D11DeviceContext *pImmediateContext)
{
    ...
    // calculate the fps string
    framesSinceLastFPSUpdate++;
    double curFrameTime = mpFPSTimer->GetElapsedTime();
    timeSinceLastFPSUpdate += curFrameTime;
    minFrameTime = std::min(minFrameTime, curFrameTime);
    maxFrameTime = std::max(maxFrameTime, curFrameTime);

    double curFPS = 1 / curFrameTime; 
    mLastFPS = (float) curFPS;

    if (1.0f <= timeSinceLastFPSUpdate)    // 1 Hz min
    {
        avgFPS = framesSinceLastFPSUpdate / timeSinceLastFPSUpdate;
        framesSinceLastFPSUpdate = 0;
        timeSinceLastFPSUpdate = 0.0f;
        minFrameTime = maxFrameTime = curFrameTime;
    ...
    if(mbDrawFPS)
    {
        ...
        cString FPSData;
        {
            ...
            swprintf_s(&wcstring[0], CPUT_MAX_STRING_LENGTH, _L("FPS Avg: %.2f, Inst: %.2f"), avgFPS, curFPS);
            FPSData=wcstring;
        }
        cString Resolution;
        {
            wchar_t wcstring[CPUT_MAX_STRING_LENGTH];
            swprintf_s(&wcstring[0], CPUT_MAX_STRING_LENGTH, _L("[%3dx%3d]  "), windowRect.width, windowRect.height);
            Resolution = wcstring;
        }
        cString MinMax;
        {
            wchar_t wcstring[CPUT_MAX_STRING_LENGTH];
            swprintf_s(&wcstring[0], CPUT_MAX_STRING_LENGTH, _L("Frame (ms): [%4.1f - %4.1f]  "), 1000*minFrameTime, 1000*maxFrameTime);
            MinMax = wcstring;
        }

        // build the FPS string
        cString FPS = Resolution+MinMax+FPSData;
...
CPUTResult CPUTGuiControllerDX11::RegisterGUIResources(ID3D11DeviceContext *pImmediateContext, cString VertexShaderFilename, cString PixelShaderFilename, cString RenderStateFile, cString DefaultFontFilename, cString ControlAtlasTexture)
{
    ...
    result = pAssetLibrary->GetVertexShader(FinalPath, _L("VS"), _L("vs_4_0"), &mpGUIVertexShader, true);
    ...
    CPUTFileSystem::ResolveAbsolutePathAndFilename(FullPath, &FinalPath);
    result = pAssetLibrary->GetPixelShader(FinalPath, _L("PS"), _L("ps_4_0"), &mpGUIPixelShader, true);
