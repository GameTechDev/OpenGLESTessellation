...
//-----------------------------------------------------------------------------
CPUTResult CPUT_DX11::MakeWindow(const cString WindowTitle, CPUTWindowCreationParams windowParams)
{
    CPUTResult result;
...
CPUTResult CPUT_DX11::CreateDXContext(CPUTContextCreation ContextParams )
{
    ...
    mSyncInterval = ContextParams.refreshRate ? 60/ContextParams.refreshRate : 0;

    // walk devices and create device and swap chain on first successful
    ...
    for( UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++ )
    {
        ...
        if( SUCCEEDED( hr ) )
        {
            ...
            if(functionalityTestPassed)
            {
                break;
            }
            else
            {
                // Context failed to have required features. Release created resources.
    ...
    ASSERT( SUCCEEDED(hr) && functionalityTestPassed, _L("Failed creating device and swap chain.") );
    if(!SUCCEEDED(hr) || !functionalityTestPassed)
    {
        CPUTOSServices::OpenMessageBox(_L("Required DirectX hardware support not present"), _L("Your system does not support the DirectX feature levels required for this sample."));
        exit(1);
    }
    ...
    CPUTAssetLibrary::GetAssetLibrary()->AddConstantBuffer( name, _L(""), _L(""), mpPerFrameConstantBuffer );
    ...
    bd.Usage = D3D11_USAGE_DYNAMIC;
    bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
    hr = (CPUT_DX11::GetDevice())->CreateBuffer( &bd, NULL, &pModelBuffer );
    ASSERT( !FAILED( hr ), _L("Error creating constant buffer.") );
    CPUTSetDebugName( pModelBuffer, _L("Model Constant buffer") );
    name = _L("$cbPerModelValues");
    mpPerModelConstantBuffer = new CPUTBufferDX11(name, pModelBuffer);
    CPUTAssetLibrary::GetAssetLibrary()->AddConstantBuffer( name, _L(""), _L(""), mpPerModelConstantBuffer );
...
void CPUT_DX11::ResizeWindow(UINT width, UINT height)
{
    ...
    else
    {
        ...
        pAssetLibrary->AddBuffer( backBufferName, _L(""), _L(""), mpBackBuffer );
    }
    ...
    else
    {
        ...
        pAssetLibrary->AddTexture( backBufferName, _L(""), _L(""), mpBackBufferTexture );
    }
    ...
    else
    {
        ...
        pAssetLibrary->AddBuffer( depthBufferName, _L(""), _L(""), mpDepthBuffer );
    }
    ...
    else
    {
        ...
        pAssetLibrary->AddTexture( DepthBufferName, _L(""), _L(""), mpDepthBufferTexture );
    }
...
void CPUT_DX11::UpdatePerFrameConstantBuffer( CPUTRenderParameters &renderParams, double totalSeconds )
{
    if( mpPerFrameConstantBuffer )
    {
        ...
        pCb->View           = view;
        pCb->InverseView    = inverseView;
        pCb->Projection     = projection;
        pCb->ViewProjection = viewProjection;
        pCb->AmbientColor   = float4(mAmbientColor, 0.0f);
        pCb->LightColor     = float4(mLightColor, 0.0f);
        pCb->LightDirection = lightDir;
        pCb->EyePosition    = eyePosition;
        pCb->TotalSeconds   = float4((float)totalSeconds);
...
void CPUT_DX11::InnerExecutionLoop()
{
    ...
    if(!mbShutdown)
    {
        ...
        CPUTMaterialEffectDX11::ResetStateTracking();
        Render(deltaSeconds);
...
// are preserved
//-----------------------------------------------------------------------------
CPUTResult CPUT_DX11::CPUTParseCommandLine(std::wstring commandLine, CPUTWindowCreationParams *pWindowParams, std::wstring *pFilename, std::wstring *pSceneFilename)
{
    ...
    std::wstring CommandLineParams(commandLine);        
    std::transform(CommandLineParams.begin(), CommandLineParams.end(), CommandLineParams.begin(), tolow);
    ...
    while(token) 
    {
        if('-' == token[0])
        {
            ...
            std::wstring ParameterName(&token[1]);
            if(0!=ParameterName.size())
            {
                ...
                else if(0==ParameterName.compare(_L("vsync")))
                {
                    ...
                    {
                        // vsync set to 30 FPS
                        pWindowParams->deviceParams.refreshRate = 60;
                    }
                    else if( (0==boolString.compare(_L("off"))) || (0==boolString.compare(_L("false"))) )
                    {
                        pWindowParams->deviceParams.refreshRate = 0;
                    }
                    else
                    {
                        pWindowParams->deviceParams.refreshRate = _wtoi(token);
                    }
                }
                ...
                else if(0==ParameterName.compare(_L("scene")))
                {
                    // get the filename
                    token = wcstok_s(NULL, separators, &nextToken);
                    pSceneFilename->assign(token);
                }
                else
                {
...
CPUTResult CPUT_DX11::CPUTParseCommandLine(std::string commandLine, CPUTWindowCreationParams *pWindowParams, std::string *pFilename, std::string *pSceneFilename)
{
    ASSERTA( (NULL!=pWindowParams), "Required command line parsing parameter is NULL");
    ASSERTA( (NULL!=pFilename), "Required command line parsing parameter is NULL");
    ...
    // we do have parameters, so parse them.
    // convert command line to lowercase version
    std::string CommandLineParams(commandLine);        
    std::transform(CommandLineParams.begin(), CommandLineParams.end(), CommandLineParams.begin(), tolow);
    ...
    char separators[]   = " \t\n";
    char* nextToken = NULL;
    char* token = strtok_s((char*)CommandLineParams.c_str(), separators, &nextToken);
    while(token) 
    {
        if('-' == token[0])
        {
            // parameter - get next token for which one
            std::string ParameterName(&token[1]);
            if(0!=ParameterName.size())
            {
                if(0==ParameterName.compare("width"))
                {
                    // get the next value
                    token = strtok_s(NULL, separators, &nextToken); 
                    ASSERT(token, _L("-width command line parameter missing required numerical value"));
                    pWindowParams->windowWidth = atoi(token);
                }
                else if(0==ParameterName.compare("height"))
                {
                    // get the next value
                    token = strtok_s(NULL, separators, &nextToken); 
                    ASSERT(token, _L("-height command line parameter missing required numerical value"));
                    pWindowParams->windowHeight = atoi(token);
                }
                else if(0==ParameterName.compare("fullscreen"))
                {
                    // get the bool 
                    token = strtok_s(NULL, separators, &nextToken); 
                    std::string boolString(token);
                    if(0==boolString.compare("true"))
                    {
                        pWindowParams->startFullscreen = true;
                    }
                }
                else if(0==ParameterName.compare("vsync"))
                {
                    // get the bool 
                    token = strtok_s(NULL, separators, &nextToken); 
                    std::string boolString(token);
                    if( (0==boolString.compare("on")) || (0==boolString.compare("true")) )
                    {
                        // vsync set to 30 FPS
                        pWindowParams->deviceParams.refreshRate = 60;
                    }
                    if( (0==boolString.compare("off")) || (0==boolString.compare("false")) )
                    {
                        pWindowParams->deviceParams.refreshRate = 0;
                    }
                }
                else if(0==ParameterName.compare("xpos"))
                {
                    // get the next value
                    token = strtok_s(NULL, separators, &nextToken); 
                    ASSERTA(token, "-xpos command line parameter missing required numerical value");
                    pWindowParams->windowPositionX = atoi(token);
                }
                else if(0==ParameterName.compare("ypos"))
                {
                    // get the next value
                    token = strtok_s(NULL, separators, &nextToken); 
                    ASSERTA(token, "-ypos command line parameter missing required numerical value");
                    pWindowParams->windowPositionY = atoi(token);
                }
                else if(0==ParameterName.compare("file"))
                {
                    // get the filename 
                    token = strtok_s(NULL, separators, &nextToken);
                    pFilename->assign(token);
                }
                else if(0==ParameterName.compare("scene"))
                {
                    // get the filename
                    token = strtok_s(NULL, separators, &nextToken);
                    pSceneFilename->assign(token);
...
#define ENABLE_GUI
    ...
    CPUTGuiControllerDX11::GetController()->SetWindow(mpWindow);
#endif
    HEAPCHECK;
    DrawLoadingFrame();
    HEAPCHECK;
    ...
    {
        CPUTOSServices::OpenMessageBox(_L("Performance warning"), _L("Your graphics hardware does not support the DirectX features required by this sample. The sample is now running using the DirectX software rasterizer."));
    }

    // Call the sample-specific Create() method
    HEAPCHECK;
    Create();
    HEAPCHECK;
    ...
    const float srgbClearColor[] = { 0.0f, 0.5f, 1.0f, 1.0f };
