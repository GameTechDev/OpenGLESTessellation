...
CPUT_OGL::CPUT_OGL() :
        ...
        mpPerModelConstantBuffer(NULL)
    {
        ...
        DEFAULT_MACROS = new CPUT_SHADER_MACRO[2];
        DEFAULT_MACROS[0].Name = "OPENGL_ES";
        DEFAULT_MACROS[0].Definition = "1";
        mpTimer = (CPUTTimer*) new CPUTTimerLinux();
...
// Handle mouse events
//-----------------------------------------------------------------------------
CPUTEventHandledCode CPUT_OGL::CPUTHandleMouseEvent(int x, int y, int wheel, CPUTMouseState state)
{
//    assert(false);
    CPUTEventHandledCode handleCode = CPUT_EVENT_UNHANDLED;
    
    // dispatch event to GUI to handle GUI triggers (if any)
    handleCode = CPUTGuiControllerOGL::GetController()->HandleMouseEvent(x,y,wheel,state);
