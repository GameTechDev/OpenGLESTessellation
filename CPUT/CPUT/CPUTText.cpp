...
void CPUTText::Recalculate()
{
    ...
    for(int ii=0; ii<mNumCharsInString; ii++)
    {
        ...
        mQuadSize.height = std::max(mQuadSize.height, size.height);
	...
	int width, height;
#ifdef UNICODE
    char *tempString = ws2s(mStaticText);
    mpFont->LayoutText(mpMirrorBuffer, &width, &height, tempString, mPosition.x, mPosition.y);
    delete tempString;
#else
