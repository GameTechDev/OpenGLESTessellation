...
D3D11_MAPPED_SUBRESOURCE CPUTBufferDX11::MapBuffer( CPUTRenderParameters &params, eCPUTMapType type, bool wait )
{
    ...
    if( type == CPUT_MAP_WRITE_DISCARD || type == CPUT_MAP_NO_OVERWRITE )
    {
        // If discard or no_overwrite, map the resource directly
        // Note: buffer must have been created as DYNAMIC
        hr = pContext->Map( mpBuffer, wait ? 0 : D3D11_MAP_FLAG_DO_NOT_WAIT, (D3D11_MAP)type, 0, &info );
        ASSERT( SUCCEEDED(hr), _L("Failed mapping buffer") );
    }
    else
    {
        // If NOT discard or no_overwrite, map a staging buffer and copy
        if( !mpBufferStaging )
        {
            // We don't already have a staging buffer, so create it.
...
void CPUTBufferDX11::UnmapBuffer( CPUTRenderParameters &params )
{
    ...
    case CPUT_MAP_NO_OVERWRITE:
        // We mapped for discard, so we didn't use a staging buffer.  Unmap the original buffer.
